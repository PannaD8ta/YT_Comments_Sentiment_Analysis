...
def get_comments(part='snippet', 
                 ...
                 videoId=videoId,
                 csv_filename="insertfilenamehere" # Insert your file name here
                 ):
...
if __name__ == '__main__':
    sirajs_topvidtwo_comments = get_comments()
    df = pd.DataFrame(sirajs_topvidtwo_comments)
    print(df.shape)
    print(df.head())
    df['date'] = pd.to_datetime(df['date'], errors='coerce')
    df['just_date'] = df['date'].dt.date
    df.to_csv('./sirajs_topvidtwo_comments.csv')
